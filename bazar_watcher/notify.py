...
def _cfg() -> dict:
    ...
    return {
        ...
        "user": os.environ.get("SMTP_USER", ""),
        "password": os.environ.get("SMTP_PASS", ""),
        "to": os.environ.get("ALERT_EMAIL", os.environ.get("SMTP_USER", "")),
    }


def _build_html_table(listings: list[dict]) -> str:
    rows = ""
    for lst in listings:
        price = f"{lst['price_eur']:.0f} €" if lst.get("price_eur") else "—"
        year = str(lst.get("year") or "—")
        cat = lst.get("category") or "—"
        src = lst.get("source_name") or lst.get("source_id") or "—"
        url = lst.get("url") or "#"
        title = lst.get("title") or "—"
        weight = lst.get("weight_range") or "—"
        size = lst.get("size") or "—"

        rows += f"""
        <tr>
          <td style="padding:6px 10px;border:1px solid #ddd;">{src}</td>
          <td style="padding:6px 10px;border:1px solid #ddd;">
            <a href="{url}" style="color:#1a73e8;">{title[:80]}</a>
          </td>
          <td style="padding:6px 10px;border:1px solid #ddd;text-align:center;">{cat}</td>
          <td style="padding:6px 10px;border:1px solid #ddd;text-align:center;">{year}</td>
          <td style="padding:6px 10px;border:1px solid #ddd;text-align:right;">{price}</td>
          <td style="padding:6px 10px;border:1px solid #ddd;text-align:center;">{size}</td>
          <td style="padding:6px 10px;border:1px solid #ddd;text-align:center;">{weight}</td>
        </tr>"""

    return f"""
    <html><body>
    <p style="font-family:sans-serif;">
      Ahoj! 🪂 Bazar watcher našel <strong>{len(listings)}</strong> nových
      Low-B křídel splňujících tvoje parametry ({date.today()}).
    </p>
    <table style="border-collapse:collapse;font-family:sans-serif;font-size:13px;width:100%;">
      <thead>
        <tr style="background:#1a73e8;color:white;">
          <th style="padding:8px 10px;border:1px solid #ddd;">Zdroj</th>
          <th style="padding:8px 10px;border:1px solid #ddd;">Inzerát</th>
          <th style="padding:8px 10px;border:1px solid #ddd;">Kategorie</th>
          <th style="padding:8px 10px;border:1px solid #ddd;">Rok</th>
          <th style="padding:8px 10px;border:1px solid #ddd;">Cena</th>
          <th style="padding:8px 10px;border:1px solid #ddd;">Vel.</th>
          <th style="padding:8px 10px;border:1px solid #ddd;">Váha</th>
        </tr>
      </thead>
      <tbody>
        {rows}
    ...
    <p style="font-family:sans-serif;font-size:12px;color:#777;margin-top:16px;">
      Parametry: EN B / Low B, rok výroby ≥ {date.today().year - 5}<br>
      Zdroje: CZ, AT, DE, CH
    </p>
    </body></html>"""


def send_alert(matching_listings: list[dict], dry_run: bool = False) -> bool:
    """
    Pošle email s novými matching inzeráty.

    Args:
        matching_listings: Filtrované new inzeráty (EN B, rok >= min_year)
        dry_run: Pokud True, jen vytiskne obsah bez odeslání

    Returns:
        True pokud email odeslán (nebo dry_run), False při chybě.
    """
    if not matching_listings:
        logger.info("Žádné nové matching inzeráty → email se neposílá.")
        return True
    ...
    if not cfg["user"] or not cfg["password"]:
        logger.warning(
            "SMTP přihlašovací údaje nejsou nastaveny (SMTP_USER / SMTP_PASS). "
            "Email se neposílá."
        )
        return False

    count = len(matching_listings)
    subject = f"🪂 Bazar křídel: {count} nový{'ch' if count > 1 else ''} Low-B inzerát{'ů' if count > 1 else ''} ({date.today()})"
    html_body = _build_html_table(matching_listings)

    if dry_run:
        print(f"\n{'='*60}")
        print(f"DRY RUN – email by byl odeslán.")
        print(f"To: {cfg['to']}")
        print(f"Subject: {subject}")
        print(f"Inzeráty ({count}):")
        for lst in matching_listings:
            print(f"  - {lst.get('source_name')} | {lst.get('title', '')[:80]} | "
                  f"{lst.get('price_eur', '?')} € | rok {lst.get('year', '?')}")
        print("=" * 60)
        return True
    ...
    # Plaintext fallback
    plain_lines = [f"Nové Low-B inzeráty ({date.today()}):\n"]
    for lst in matching_listings:
        plain_lines.append(
            f"- {lst.get('source_name')} | {lst.get('title', '')[:80]} | "
            f"{lst.get('price_eur', '?')} € | rok {lst.get('year', '?')}\n"
    ...
    try:
        ...
        if port == 587:
            # STARTTLS
            context = ssl.create_default_context()
            with smtplib.SMTP(cfg["server"], port) as smtp:
                smtp.starttls(context=context)
                smtp.login(cfg["user"], cfg["password"])
            ...
            # SSL (port 465)
            context = ssl.create_default_context()
            with smtplib.SMTP_SSL(cfg["server"], port, context=context) as smtp:
                smtp.login(cfg["user"], cfg["password"])
                smtp.sendmail(cfg["user"], cfg["to"], msg.as_string())

        logger.info("Email odeslán na %s (%d inzerátů)", cfg["to"], count)
        return True

    except smtplib.SMTPException as exc:
        logger.error("Odeslání emailu selhalo: %s", exc)
        return False
